...
public class DefaultCollectionValidatorTest {
    ...
    public void testValidate_nullCollection_defaultBehavior_returnsFailure() {
        CollectionValidator<String, List<String>> validator = new DefaultCollectionValidator(LIST_CLASS, String.class);
    ...
    public void testValidate_withCollectionRule_andElementRule_combinedFailures() {
        CollectionValidator validator = new DefaultCollectionValidator(LIST_CLASS, String.class);
        validator.addRule(ValidationRules.minsize(3));
        validator.elementRule((java.util.function.Predicate<String>) s -> !s.isEmpty(), "element is empty");
    ...
    public void testValidate_nullCollection_elementValidatorNotCalled() {
        CollectionValidator validator = new DefaultCollectionValidator(LIST_CLASS, String.class);
        validator.elementRule((java.util.function.Predicate<String>) s -> s != null, "element is null");
        List<ValidationResult> results = validator.validate(null);
        assertThat(results).hasSize(1);
        assertThat(results.get(0).getErrorMessage()).isEqualTo("对象不能为null");
    }
    ...
    public void testElementRule_withPredicate() {
        CollectionValidator<Integer, List<Integer>> validator = new DefaultCollectionValidator((Class) List.class, Integer.class);
